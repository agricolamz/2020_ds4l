---
title: "6. Уменьшение размерности: PCA, LDA"
author: "Г. Мороз"
editor_options: 
...
```{r}
wodehouse_2 <- as.data.frame(wodehouse[,-1])
rownames(wodehouse_2) <- wodehouse$chapter
...
PCA чувствителен к дисперсии в переменных, т. е. если дисперсия в одной переменной будет большой, то PCA расположит эту переменную на PC1 и т. п. Чтобы этого избежать можно использовать аргумент `scale.`
...
### 7. Евангелия
```{r, eval = FALSE}
gospels <- read_csv("https://tinyurl.com/y8tcf3uw")
```
